...
/*   Updated: 2025/05/03 17:44:37 by adeters          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	act_manipulate_think(t_philo *p)
{
	...
	if (p->philo_nb % 2 == 0 && p->times_eaten == 0 && ft_usleep(30000,
			p->data))
		return ;
	if (p->data->nbp % 2 != 0 && p->philo_nb == get_next_delay(p->data))
	{
		...
		if (ft_usleep(20000 + extra, p->data))
			return ;
...
int	act_think_and_sleep(t_philo *p)
{
	if (!p_log(p->data, p->philo_nb, SLEEP, p->mutexes))
		return (0);
	if (ft_usleep(p->data->tts, p->data))
		return (0);
