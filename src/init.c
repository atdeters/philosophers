...
/*   Updated: 2025/04/29 01:55:12 by andreas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo	*constructor(t_data *data, pthread_mutex_t *mutexes, int nb)
{
	...
	p->time_since_meal = 0;
	p->times_eaten = 0;
	// pthread_mutex_lock(&mutexes[data->nbp + 4]); //! Does not yet exist
	p->has_finished = false;
	// pthread_mutex_unlock(&mutexes[data->nbp + 4]); //! Does not yet exist
	p->mutexes = mutexes;
	if (nb == 1)
